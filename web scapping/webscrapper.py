...
def fetch_image_urls_util(url, driver_path):
    ...
    with webdriver.Chrome(service=Service(ChromeDriverManager().install())) as wd:
        ...
        #thumbnail_results = wd.find_elements_by_css_sethumbnail_resultslector("img[class ='irc_mi']")
        thumbnail_results=wd.find_elements(By.CSS_SELECTOR, "irc_mi")
        #for img in thumbnail_results:
        print(thumbnail_results[0])
        if thumbnail_results[0].get_attribute('src') and 'http' in thumbnail_results[0].get_attribute('src'):
            images.append(thumbnail_results[0].get_attribute('src'))
    return images


def fetch_image_urls(query: str, max_links_to_fetch: int, wd, sleep_between_interactions: int = 1, driver_path=None,
                     target_path=None, search_term=None):
    ...
    while image_count < max_links_to_fetch:
        ...
        thumbnail_results = wd.find_elements(By.CLASS_NAME,"rg_i Q4LuWd" )
