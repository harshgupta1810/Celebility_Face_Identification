...
def app():
    ...
    if image_file is not None:
        ...
        if image_processed is not None:
            ...
            st.image(image_processed, use_column_width=True)

            # Make predictions using the loaded model
            prediction = __model.predict(np.array([image_processed.flatten()]))
            pred_class = prediction.argmax()
            st.subheader("Prediction")
            st.write(pred_class)
        else:
            st.error("No face with two eyes detected in the uploaded image.")
